  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      ...
      <div className="p-3 sm:ml-56 sm:p-6">
          ...
          {(["today", "7days", "30days", "custom"] as Preset[]).map((p) => (
            <button
              ...
              style={{
                ...
                border: preset === p ? "none" : "0.5px solid #3f3f46",
              }}
            >
              {p === "today" && "Hari Ini"}
              {p === "7days" && "7 Hari"}
          ...
          {preset === "custom" && (
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="rounded-md border border-neutral-700 bg-neutral-900 px-2 py-1 text-[11px] text-neutral-300 sm:text-xs"
              />
              <span className="text-[10px] text-neutral-500">s/d</span>
              <input
                type="date"
                value={customTo}
        ...
        {/* Date range display */}
        {data && (
          <p className="mb-4 text-[10px] text-neutral-500 sm:text-xs">
        ...
        {data && data.byLogam.length > 0 && (
          <div className="mb-6 rounded-lg border border-neutral-800 p-3 sm:p-4">
            <h3 className="mb-3 text-xs font-medium sm:text-sm">Breakdown per Logam</h3>
            <div className="space-y-2">
              {data.byLogam.map((item) => (
                <div key={item.logam_id} className="flex items-center justify-between border-b border-neutral-800 pb-2 last:border-0">
                  <div className="flex items-center gap-2">
                    <div
                    ...
                    <span className="text-[10px] text-neutral-500">({item.jumlah} trx)</span>
                  </div>
          ...
          <div className="rounded-lg border border-neutral-800 p-3 sm:p-4">
            ...
            <div className="hidden overflow-x-auto sm:block">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {data.daily.map((d) => (
                    <tr key={d.tanggal} className="border-b border-neutral-800/50 last:border-0">
                      ...
                      <td className="py-2 text-right text-neutral-400">{d.jumlah}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="space-y-2 sm:hidden">
              {data.daily.map((d) => (
                <div key={d.tanggal} className="rounded-lg bg-neutral-900 p-2.5">
                  ...
                  <div className="text-[10px] text-neutral-500">{d.jumlah} transaksi</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Empty state */}
        {data && data.daily.length === 0 && (
          <div className="rounded-lg border border-neutral-800 py-8 text-center">
            <div className="mb-2 text-2xl">📊</div>
            <p className="text-[10px] text-neutral-500 sm:text-xs">Belum ada transaksi di periode ini</p>
          </div>
        )}
      </div>
    </div>
...
function SummaryCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="rounded-lg border border-neutral-800 p-3 sm:p-4">
      <div className="text-[9px] uppercase tracking-wider text-neutral-500 sm:text-[10px]">{label}</div>
