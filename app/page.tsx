...
export default function Home() {
  ...
  return (
    <AppShell hideMobileHeader>
      {/* Top bar — desktop shows info+menu, mobile keeps its own topbar */}
      <div className="sticky top-0 z-40 flex items-center justify-between border-b t-border px-3 py-2.5 backdrop-blur-md sm:px-4"
        ...
        <div className="flex items-center gap-2.5">
          <BrandMark className="h-8 w-8 text-base sm:hidden" />
          <span className="font-display max-w-[130px] truncate text-base font-semibold leading-none t-text-1 sm:max-w-none sm:text-base">
            {tenant.nama_toko}
          </span>
          ...
          {showMenu && (
            <div className="absolute right-0 top-full z-50 mt-1 w-40 rounded-md border t-border-md t-bg-card shadow-lg">
              ...
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2 text-xs t-text-2 t-bg-hover sm:hidden"
              >
      ...
      {/* Main POS — add bottom padding on mobile for bottom nav */}
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-30 border-t t-border backdrop-blur-md sm:hidden safe-bottom"
